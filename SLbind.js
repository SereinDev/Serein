...
//let opqqlist = /.*?"PermissionList":\[(.*?)\].*/;
let opqqlist = JSON.parse(settings).Bot.PermissionList;
//let opqq = settings.replace(opqqlist, "$1");
let opqq = opqqlist;
...
if (unbang.Group_Decrease.length == 2) {
    unbang.Group_Decrease.splice(0, 2, "g|玩家%ID%退出群聊 已删除白名单，已自动解绑游戏ID\nԾ‸ Ծ", "s|whitelist remove \"%GameID:%ID%%\"", "unbind|%ID%");
